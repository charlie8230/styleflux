...
const _processCssBody = (bodyContent, presets) => {
  ...
  bodyContentArr.forEach((attribute) => {
    if (attribute.length > 1) {
      ...
      if (pullColorVar != null) {
        ...
        let filteredColors = arr.filter(item=>{
          let rx = item['value'].toUpperCase();
          let found = colorVar.toUpperCase();
          return rx.search(found) >=0;
        }).map(item=>{
          return item['key'];
        });
        if (filteredColors.length>0) {
          presetName = filteredColors[0];
          _colorObj[`$${presetName}`] = `${colorVar}`;
        } else {
          presetName = colorName;
          _colorObj[`$${colorName}`] = `${colorVar}`;
        }
        
        //console.log(colorVar, colorName);
        modAttribute = attribute.replace(/([^0-9A-Za-z]+)(#[0-9A-Fa-f]{3,6})/, `$1$${presetName}`);
      }
...
const _cssarrayToObject = (fileContentsArr, presets) => {
  const mainObject = {};
    ...
    dividedHead.forEach((headvalue) => {
      if (head.length > 0) {
        ...
        if (processedHead.substr(0, 2) === '"@') {
          ...
          processedBody = _processCssBody(tail, presets);
        }
...
const convertCssToObject = (cssContent, presets) => {
  let plainCss;
  try {
  ...
  try {
    return _cssarrayToObject(cssArray, presets);
  } catch (error) {
...
const convertCssToScss = (cssContent, presets) => {
  _mainSCSS = '', _mainCharset = '', _colorObj = {};
  const cssObject = convertCssToObject(cssContent, presets);
  _cssObjectToCss(cssObject);
  const charset = _mainCharset ? `@charset "${_mainCharset}";\n` : '';
  const colorVars = _objToKeyValueCss(_colorObj, presets);
